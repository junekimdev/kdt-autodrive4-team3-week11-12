...
# Get Image Path List
image_path_list = glob.glob("checker_board_final/*.jpg")
...
tmp_image = cv2.imread("checker_board_final/left0080.jpg", cv2.IMREAD_ANYCOLOR)
...
#camera_matrix_init = np.array([[422.03786, 0.00000, 245.89540],[0.00000, 435.58973, 163.62554],[0.00000, 0.00000, 1.00000]])
#dist_init = np.array([[-0.28930, 0.06103, 0.00179, 0.01524, 0.00000 ]])


newcameraMtx, roi = cv2.getOptimalNewCameraMatrix(camera_matrix, dist_coeffs, (image_width, image_height), 1, (image_width, image_height))
dst = cv2.undistort(tmp_image, camera_matrix, dist_coeffs, None, newcameraMtx)
x, y, w, h = roi
dst = dst[y:y+h, x:x+w]
