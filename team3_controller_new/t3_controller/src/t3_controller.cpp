 ...
 */ 
  void Controller::control()
  {
    ...
    if (new_id == -1){
      if (sensor_state_.cam.left_detected == true && sensor_state_.cam.right_detected == false ){
      rpos_PID = lpos_PID +358;
      }
      else if (sensor_state_.cam.left_detected == false && sensor_state_.cam.right_detected == true ){
    ...
    */
    ...
    if (new_id == 2 || new_id == 3){
      ...
      if (!sensor_state_.cam.left_detected) angle -= 5;
      if (!sensor_state_.cam.right_detected) angle += 5;
      angle = correctAngle(angle);
      control_state_.reduce(mode, angle, 10);
      }
    }
