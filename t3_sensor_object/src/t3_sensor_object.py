...
NAME = "yolov3-trt"
FILE_DIR = "/home/nvidia/xycar_ws/src/t3_controller/t3_sensor_object/src"
...
camera_matrix= np.array([[348.14820298,0.,323.82585721], [0.,345.88660809, 260.82857638], [0.,0.,1.]])
dist_coeffs = np.array([[-0.3460703,0.14435681,0.00177496,0.00118325,-0.0321834]])


def calculate_azimuth(bbox):
  cx, cy, w, h = int(bbox[0]*640), int(bbox[1]*480), int(bbox[2]*640), int(bbox[3]*480)
  azimuth = (cx - 320)*FOV_H/640
  return azimuth

def distance(azimuth):
  distance = (FOCAL_LENGTH * 2 / math.tan(azimuth/2)) / (2/math.tan(azimuth/2)-FOCAL_LENGTH)
  return distance * math.cos(azimuth), distance * math.sin(azimuth)


def img_callback(data):
    global xycar_image
    img = np.frombuffer(data.data, dtype=np.uint8).reshape(data.height, data.width, -1)
    newcameraMtx, roi = cv2.getOptimalNewCameraMatrix(camera_matrix, dist_coeffs, (data.width, data.height), 1, (data.width, data.height))

    xycar_image = cv2.undistort(img, camera_matrix, dist_coeffs, None)
    xycar_image = cv2.cvtColor(xycar_image, cv2.COLOR_RGB2BGR)
...
class yolov3_trt(object):
    def __init__(self):
        ...
        self.engine_file_path = os.path.join(FILE_DIR, "model_epoch500.trt")
